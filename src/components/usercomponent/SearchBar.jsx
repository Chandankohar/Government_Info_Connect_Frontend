      ...
      setSearchTimeout(
        setTimeout(async () => {
          const { data } = await axiosInstance.get(
            `/scheme/usersearch/${searchText.trimStart()}`,
          );
    ...
    <>
      <div className="flex w-4/6 overflow-hidden rounded-full border border-gray-400 bg-gray-300 shadow-sm hover:shadow-lg md:w-1/2">
        <div className="grow">
          <input
            type="search"
            placeholder="Search Scheme Here"
            className="h-full w-full border-none py-2 px-4 text-sm text-black   focus:outline-none md:text-lg"
